...
do
    ...
    if [[ $line == *":"* ]]; then
        ...
        test=${testArr[0]}
        index=${testArr[1]}
        echo "first = adb -s $device shell am instrument -w -e class $test -e paramIndex $index $package.test/$testInstrumentationRunner > $runningTest"
        adb -s $device shell am instrument -w -e class $test -e paramIndex $index $package.test/$testInstrumentationRunner > $runningTest
    else
        # run as usual
        echo "$line"
        startTime=$(($(date +'%s * 1000 + %-N / 1000000')))
        echo "second - adb -s $device shell am instrument -w -e class $line $package.test/$testInstrumentationRunner > $runningTest"
        adb -s $device shell am instrument -w -e class $line $package.test/$testInstrumentationRunner > $runningTest
        endTime=$(($(date +'%s * 1000 + %-N / 1000000')))
